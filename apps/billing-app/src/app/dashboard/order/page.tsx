...
const Page = () => {
  ...
  const columns = [
    ...
    {
      header: "Order Date",
      accessor: "orderedDate" as keyof PurchaseOrderData,
    },
    // {
    //   header: "Pharmacy",
    ...
    },
    {
      ...
      accessor: "intendedDeliveryDate" as keyof PurchaseOrderData,
    },
    {
    ...
    },
    {
      ...
      accessor: (row: PurchaseOrderData, index: number) => (
  ...
  useEffect(() => {
    const fetchPurchaseOrder = async () => {
      try {
        ...
        setPurchaseOrderData(purchaseOrderWithDetails);
      } catch (error) {
  ...
  const filteredData = purchaseOrderData.filter((item) => {
    ...
    return (
      ...
      new Date(item.orderedDate).toLocaleDateString().toLowerCase().includes(search) ||
      item.supplierName?.toLowerCase().includes(search) ||
      new Date(item.intendedDeliveryDate).toLocaleDateString().toLowerCase().includes(search) ||
  ...
  return (
    <>
      ...
      {showPurchasOrder && (
        <PurchaseOrder setShowPurchasOrder={setShowPurchasOrder} />
      )}
    </>
  );
};
