...
const PurchaseReturn: React.FC<PurchaseReturnProps> = ({
  ...
  //   if (["itemName", "returnType"].includes(id)) {
  //     setFormData((prev) => {
  //       const updatedItems = [...prev.purchaseReturnItemDtos];
  //       if (!updatedItems[0]) updatedItems[0] = {} as any;

  //       (updatedItems[0] as any)[id] = value;
  ...
  //   }
  //   else if (
  //     name.startsWith("discrepancy") ||
  //     name.startsWith("discrepancyIn")
  //   ) {
  ...
  //     setFormData((prev) => {
  //       const updatedItems = [...prev.purchaseReturnItemDtos];
  //       if (!updatedItems[index]) updatedItems[index] = {} as any;

  //       updatedItems[index][field as "discrepancy" | "discrepancyIn"] = value;
  ...
  //     setFormData((prev) => ({
  //       ...prev,
  //       [id]: id === "returnDate" ? new Date(value) : value,
  //     }));
  //   }
  // };
  ...
  const handleInputChange = (
    ...
    // 1) If name is "someField-<index>", update that line item:
    if (parts.length === 2) {
      // parts[0] is the field name, parts[1] is the index
      const field = parts[0] as keyof PurchaseReturnItem;
      const idx = Number(parts[1]);
      // convert to number if it's the quantity field
    ...
    // 2) Otherwise it's a top‑level field on formData:
    else {
