...
// AdminStatsController on the backend authenticates via a mandatory
// `Authorization` header rather than the httpOnly accessToken cookie every
// other endpoint uses (same pattern as SuperAdminStatsController - see
// src/app/api/superadmin-stats/[...path]/route.ts). The browser can't read
// that cookie to build the header itself, so this route runs on the Next.js
// server, which receives the cookie regardless, and re-attaches it as a
// Bearer header while proxying through to the backend.
...
// AdminStats.tsx's Promise.allSettled fan-out of calls all land with an
// expired accessToken at once, they must not each independently hit
// /auth/refresh - the second call would invalidate the first's rotated
