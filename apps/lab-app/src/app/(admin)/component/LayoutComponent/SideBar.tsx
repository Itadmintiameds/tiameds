...
const SideBar = ({ isOpen, setIsOpen }: SideBarProps) => {
  ...
  const hasAccess = (allowedRoles?: string[]) =>
    !allowedRoles || allowedRoles.length === 0 || allowedRoles.some(role => roles.includes(role));

  const filteredNavigation = navigation
    .filter(item => hasAccess(item.allowedRoles))
    .map(item => ({
      ...item,
      children: item.children?.filter(child => hasAccess(child.allowedRoles)),
    }))
    // A group with no href of its own is just a container for its children (e.g. "Package
    // Management") -- if every child got filtered out there's nothing left to click, so drop it.
...
// old code dated ...06.08.2026 with authorised page showing..........
