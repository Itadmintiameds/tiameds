...
const Result = ({
  ...
  return (
    <div className="rounded-xl border border-pneutral-200 bg-white shadow-sm">
      {/* Header */}
      <div className="border-b border-pneutral-200 px-6 py-4">
        <div className="flex items-start">
          <div className="flex items-center justify-center ">
            <CiMedicalClipboard className="h-5 w-5 mt-3 text-secondary-700" />
          </div>
          <div>
            <h2 className="text-p4 font-semibold px-1 text-pneutral-900">
              Result Configuration
            </h2>
            <p className=" px-1 text-p2 text-pneutral-400">
              Configure how test results will be entered and interpreted
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8 p-6">
        ...
        <div>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            {resultTypes.map((title) => {
              const active =
                selectedResultType === title;
              return (
                <button
                  key={title}
                  onClick={() =>
                    handleResultType(title)
                  }
...
{selectedResultType === "Textarea" && (
  <TextArea1 />
)}

{selectedResultType === "Positive / Negative" && (
  <PositiveNegative />
)}

{selectedResultType === "Yes / No" && (
  <YesNo />
)}

{selectedResultType === "Set Range" && (
  <SetRange />
)}
{selectedResultType === "Multi-Parameter" && (
  <MultiParameter />
)}
      </div>
    </div>
