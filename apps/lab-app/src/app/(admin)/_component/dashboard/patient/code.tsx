...
const AddPatient = () => {
  ...
  useEffect(() => {
    // Calculate total amount from selected tests and packages
    ...
    // Split GST into CGST, SGST (if applicable), or IGST
    const cgstAmount = gstAmount / 2; // For intra-state transactions
    const sgstAmount = gstAmount / 2; // For intra-state transactions
    const igstAmount = gstAmount; // For inter-state transactions, or you can add logic for state-based determination

    // Calculate the net amount
    const netAmount = discountedAmount + gstAmount; // Add GST to discounted amount to get the net amount

    // Update the billing details in the patient's state
    setNewPatient((prevPatient) => ({
      ...prevPatient,
      visit: {
        ...prevPatient.visit,
        billing: {
          ...prevPatient.visit?.billing,
