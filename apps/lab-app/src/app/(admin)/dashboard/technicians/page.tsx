...
const tabs: TechnicianTab[] = [
  {
    id: 'Add Technician',
    label: 'Add Member',
    icon: <FaPersonCirclePlus className="text-xl" />,
  },
  {
    id: 'Manage Technicians',
    label: 'Manage Member',
    icon: <FaPersonChalkboard className="text-xl" />,
  },
];
...
const Page = () => {
  ...
  const isAllowed = ['ADMIN', 'SUPERADMIN'].some(role =>
    roles?.includes(role)
  );
    ...
    <div className="w-full bg-secondary-50 p-4 rounded-lg">
      <div className="w-full">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`
                  flex items-center gap-2.5 px-4 py-1 h-10 rounded-full
        ...
        <div className="mt-4">
          ...
          {selectedTab === 'Manage Technicians' && (
            <ListOfMemberOfLab />
          )}
        </div>
      </div>
