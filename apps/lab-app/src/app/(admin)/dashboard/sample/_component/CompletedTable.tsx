...
const CompletedTable: React.FC<CompletedTableProps> = ({ 
  ...
  // The backend returns "Collected" + "Completed" visits combined and matches search
  // against name/phone/patient code/visit code; the "at least one completed test" filter
  // and sort-by below still only apply to the current page, not the whole date range.
  const fetchVisits = useCallback(async (page: number) => {
    ...
    try {
      ...
      const completedVisits = response.data.filter(visit => {
  ...
  }, [fetchVisits, currentPage, refreshTrigger]);
      ...
      <div className="mt-5 rounded-xl border border-pneutral-200 bg-white p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative flex-1 max-w-xl">
            ...
            <input
              type="text"
              placeholder="Search by name, phone, patient code, or visit code"
              value={searchInput}
        ...
        {filteredPatients.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-pneutral-200 bg-white py-16">
            ...
            <p className="mt-4 text-sm font-medium text-pneutral-500">
              {searchInput ? `No results found for "${searchInput}"` : "No completed tests found"}
            </p>
            {searchInput && (
      ...
      {editModel && editPatient && editTest && editReportId !== null && (
        ...
        >
          <Editreport
            ...
            refreshReports={() => fetchVisits(currentPage)}
          />
        </NewModal>
      )}
    </div>
