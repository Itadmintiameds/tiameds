...
const [expandedSections, setExpandedSections] = useState<{
  ...
  useEffect(() => {
    if (onDataUpdate) {
      onDataUpdate(totalElements);
    }
  }, [totalElements, onDataUpdate]);
  ...
  const fetchVisits = useCallback(async (page: number) => {
    ...
    try {
      ...
      const normalizedVisits = visits.map((visit: Patient) => ({
        ...visit,
        visitDetailDto: {
      ...
      const packageIds = visits.flatMap((visit: Patient) => visit.visitDetailDto.packageIds);
    ...
    return (
      <div className="w-[274px] space-y-2">
        {row.visitDetailDto.packageIds.map((packageId) => {
          ...
          const packageKey = `package-${pkg.id}-${row.id}`;
      ...
      {tests.length > 0 && (
        <div>
          {tests.length === 1 ? (
            ...
            (() => {
              const individualKey = `individual-${row.id}`;
